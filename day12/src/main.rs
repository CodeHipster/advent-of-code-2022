...
fn main() {
  ...
  // Back track from end to a lowest point.
  let mut cont = true;
  let mut least = area.grid.cols() * area.grid.rows();
  // steps, Point, height
  let start_height = area.grid.get(end.row, end.col).unwrap().height;
  let start_steps = 0;
  let start_pos = end.clone();
  ...
  while cont {
    ...
    if path_point.height == 0 {
      // reached a lowest point.
      // println!("reached lowest point in {} steps", path_point.steps);
...
fn check_side(grid: &Grid<Path>, side: (i8, i8), current: &PathPoint) -> Option<PathPoint> {
  ...
  if let Some(target) = path_point {
    if current.height <= (target.height + 1) {
      // we can make this step. (going in the inverse direction, so infinitely up is ok.)
